...
sub cmd_keepnick {
	...
	if ($param =~ /^-(chat|irc)?net$/i) {
	    ...
	    Irssi::print("%M•%K Unknown parameter:%m $param");
	} else {
	    push @opts, $param;
	}
    }
	...
	unless ($cn) {
	    Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'keepnick_crap', 
			       "%M•%K Unknown chat network:%m $chatnet");
	    return;
	}
	$chatnet = $cn->{name};
	...
	Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'keepnick_crap', 
			   "%R•%r Not connected%K to a %rserver");
	return;
    }
...
[
 ...
 'keepnick_add',
 '{line_start}%G• {hilight Keepnick:}%K Now keeping%g {nick $0}%K on%g [$1]',

 'keepnick_remove',
 '{line_start}%R• {hilight Keepnick:}%r Stopped trying to keep {nick $0}%K on%r [$1]',

 'keepnick_hold',
 '{line_start}%R• {hilight Keepnick:}%K Nickkeeping%r deactivated%K on%r [$1]',

 'keepnick_unhold',
 '{line_start}%G• {hilight Keepnick:} Nickkeeping reactivated on [$1]',

 'keepnick_list_empty',
 '{line_start}%G• {hilight Keepnick:}%K No nicks in keep list',

 'keepnick_list_header',
 '',

 'keepnick_list_line',
 '{line_start}%G• {hilight Keepnick:}%K Keeping%g {nick $0}%K in%g [$1] ($2)',

 'keepnick_list_footer',
 '',

 'keepnick_got_nick',
 '{hilight Keepnick:} Updating Nick left [$1], returns with linked nick: {nick $0} ;)',
