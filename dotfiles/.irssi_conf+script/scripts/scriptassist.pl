...
sub print_rate (%) {
    ...
    foreach my $script (sort keys(%data)) {
	if ($data{$script}) {
            $line .= "%g•%n %9".$script."%9 has been rated";
        } else {
            $line .= "%r•%n %9".$script."%9 : Already rated this script";
        }
    }
...
sub print_debug (%) {
    ...
    my $line;
    foreach my $script (sort keys %data) {
	$line .= "%r•%n %9".$script."%9 failed to load\n";
...
sub print_install (%) {
	...
	if ($data{$script}{installed} == 1) {
		...
		$line .= "%g•%n %9".$script."%9 installed\n";
		push @installed, $script;
	    } elsif (lc($script) eq lc($IRSSI{name})) {
		$line .= "%y•%n %9".$script."%9 installed, please reload manually\n";
	    } else {
    		$line .= "%r•%n %9".$script."%9 fetched, but unable to load\n";
	    ...
	    $line .= "%r•%n %9".$script."%9 already loaded, please try \"update\"\n";
	} elsif ($data{$script}{installed} <= 0) {
	    $line .= "%r•%n %9".$script."%9 not installed\n";
    	    foreach (split /\n/, check_sig($data{$script})) {
		$line .= "  ".$_."\n";
	    }
	} else {
	    $line .= "%r•%n %9".$script."%9 not found on server\n";
	}
	$text .= $line;
    }
...
sub print_search ($%) {
	...
	my $line;
	$line .= "%g•%n" if $data{$_}{installed};
	$line .= "%y•%n" if not $data{$_}{installed};
	$line .= " %9".$_."%9 ";
	$line .= $data{$_}{desc};
...
sub print_update (%) { 
	...
	if ($data{$_}{installed} == 1) {
	    ...
	    push @table, ['%y•%n', '%9'.$_.'%9', 'upgraded ('.$local.'->'.$remote.')'];
	    foreach (split /\n/, check_sig($data{$_})) {
		push @table, ['', '', $_];
	    }
	    ...
	    push @table, ['%y•%n', '%9'.$_.'%9', 'not upgraded'];
            foreach (split /\n/, check_sig($data{$_})) {
		push @table, ['', '', $_];
            } 
	} elsif ($data{$_}{installed} == -2 && $verbose) {
	    my $local = $data{$_}{local};
	    push @table, ['%g•%n', '%9'.$_.'%9', 'already at the latest version ('.$local.')'];
    	}
    }
...
sub print_check (%) {
	...
	if (Irssi::settings_get_bool('scriptassist_check_verbose')) {
	    push @table, ['%g•%n', '%9'.$_.'%9', 'Up to date. ('.$local.')'] if $state eq 'equal';
	}
	push @table, ['%m•%n', '%9'.$_.'%9', "No version information available on network."] if $state eq "noversion";
	push @table, ['%m•%n', '%9'.$_.'%9', 'No header in script.'] if $state eq "noheader";
	push @table, ['%b•%n', '%9'.$_.'%9', "Your version is newer (".$local."->".$remote.")"] if $state eq "newer";
	push @table, ['%r•%n', '%9'.$_.'%9', "A new version is available (".$local."->".$remote.")"] if $state eq "older";;
    }
    $text = array2table(@table);
...
print CLIENTCRAP '%K----------- %G• %g'.$IRSSI{name}.' %K'.$VERSION.' %gloaded.%K For help do %g"/scriptassist help"';
