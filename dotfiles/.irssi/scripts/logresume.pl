...
Irssi::print "%G•%g $IRSSI{name}%K version%g $VERSION loaded,%K see the %gtop of%K the %gscript for help%K";
...
sub witem_created {
  ...
  if( $witem->{type} eq 'QUERY' ) {
    ...
    debug_print( "%R• %runknown%K window type%r $witem->{type}%K -- let the script author know if you get this and it's annoying" );
    return;
  }
...
sub get_log_filename {
  ...
  unless( @logs ) {
    debug_print( "%R• %rNo logfile%K for%r $tag, $name%K" );
    return undef;
  }

  debug_print( "%R• %rsurplus logfile%K for %r$tag, $name: $_ %K" ) for @logs[1..$#logs];
  return $logs[0];
}
