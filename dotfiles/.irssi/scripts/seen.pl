...
our $database_old = Irssi::get_irssi_dir . "/seen.dat.old";
...
Irssi::command_bind "seen", sub {
    ...
    if ($args =~ /^ *([^ ]+) *$/) {
        ...
        Irssi::print "%M¿ %KUsage: %m/seen <nick>";
        return;
    }
    unless ($server && $server->{connected}) {
        Irssi::print "%R¿ %K %rNot connected%K to%r server";
        return;
    }
    my ($reply, $found, $remember_asked) =
...
Irssi::command_bind "say_seen", sub {
    ...
    if ($args =~ /^ *([^ ]+) *$/) {
        ...
        Irssi::print "%M¿ %KUsage:%m /say_seen [<to_whom>] <nick>";
        return;
    }
    unless ($server && $server->{connected}) {
        Irssi::print "%R¿ %K %rNot connected%K to%r server";
    ...
    unless ($target) {
        Irssi::print "%R¿ %K %rNot%K in a%r channel%K or%r query";
        return;
    }
    my $can_show =
...
sub cmd_listen_switch($$$$) {
    ...
    if ($args =~ /^ *$/) {
        ...
        unless ($target && $target->{type} eq 'CHANNEL') {
            Irssi::print "%R¿ %K %rNot%K in a%r channel";
            return;
        }
