...
my %services = (
  ...
  qr|http://(?:www\.)?nopaste.com/(\d+)/|        => "http://nopaste.com/p/%id/txt",
  qr|http://(?:www\.)?pastebin.com/(\d+)|        => "http://pastebin.com/pastebin.php?dl=%id",
  qr|http://(?:www\.)?paste2.org/(\d+)|          => "http://pastie.org/p/%id",
  qr|http://(?:www\.)?paste.lisp.org/new/(\d+)|  => "http://paste.lisp.org/display/%id",
  qr|http://(?:www\.)?pastie.textmate.org/(\d+)| => "http://pastie.textmate.org/private/%id.txt"
);
...
sub public_message {
    ...
    for my $url_regex (keys %services) {
        if ($text =~ $url_regex) {
            ...
            if ($auto) {
                $str = "%K----------- %G•%g Autofetching%K posted%g snippet%n";
            } else {
                $str = "%K----------- %G•%g Snippet: %U$&%n [%9" . $lines . " " . ($lines == 1 ? "line" : "lines") . "%n]";
                #$str .= "\n%K-----------  %c/snip%n to open in a new window, %c/snip here%n to append to the current window";
                $str .= " %c/snip%n = new window, %c/snip here%n = current window";
