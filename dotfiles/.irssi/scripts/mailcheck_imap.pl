...
sub setup_socket {
	...
	eval {
		alarm 30;
		Irssi::print("%G•%Kmailcheck_imap%g connecting%K to%g mail server...%K");
		...
		Irssi::print("%M•%Kmailcheck_imap%m checked%K to%m mail%K");
		receive();
		alarm 0;
	};
	if ($@) {
...
sub cleanup {
  ...
  Irssi::print("%R•%Kmailcheck_imap%r exited%K the%r mail server%K");
}
